...
namespace AZ
{
    namespace Render
    {
        ...
        void PassEntry::PropagateDeviceIndex(PassEntry* childEntry, int deviceIndex)
        {
            m_childrenDeviceIndices.insert(deviceIndex);
            if (m_parent)
            {
                m_parent->PropagateDeviceIndex(this, deviceIndex);
            }
        }
            ...
            for (auto& [passName, passEntry] : timestampEntryDatabase)
            {
                ...
                if (m_passFilter.PassFilter(passEntry.m_name.GetCStr()))
                {
                    if (passEntry.m_parent && !passEntry.m_linked)
                    {
                        ...
                        passEntry.m_parent->PropagateDeviceIndex(&passEntry, passEntry.m_deviceIndex);
                    }
